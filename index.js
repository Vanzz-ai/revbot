...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6282132486898:+62 821-3248-6898\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Akhirnya beban group berkurang 1,bye bye🏃 @${num.split('@')[0]}jasa mu akan di kubur dalam²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6282132486898@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('tag orangya yang mau di jadiin admin!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Selamat, jabatan anda naik jadi admin 🥳🥳🥳 :\n'
			for (let _ of mentioned) {
			    teks += `@${_.split('@')[0]}\n`
			}
			mentions(teks, mentioned, true)
			...
			mentions(`Selamat, jabatan anda naik menjadi admin🥳🥳🥳: @${mentioned[0].split('@')[0]}`, mentioned, true)
			client.groupMakeAdmin(from, mentioned)
		    }
		    break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'tuh owner kami >_<, jangan di spamm ya..awas loh,gw culik entar 🏃🏃',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'usernamenya mana omm? ', text, {quoted: mek})
                                                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`di butuhkan nomor\nContoh : ${prefix}infonomor 08123456789...`)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                ...
                break
                   ...
                   hasil = `Negara : ${data.result.country}\n\nActive : ${data.result.active}\ncasesPerOneMillion : ${data.result.casesPerOneMillion}\ncritical : ${data.result.critical}\nmeninggal : ${data.result.deathsPerOneMillion}\nsembuh : ${data.result.recovered}\ntestPerOneMillion : ${data.result.testPerOneMillion}\ntodayCases : ${data.result.todayCases}\ntodayDeath : ${data.result.todayDeath}\ntotalCases : ${data.result.totalCases}\ntotalTest : ${data.result.totalTest}`
                   reply(hasil)
                   break
				case 'wiki':
					if (args.length < 1) return reply('yang di cari apa kak')
					tels = body.slice(6)	
                                        if (!isUser) return reply(mess.only.daftarB)				
					...
					break	
				case 'wikien':
					if (args.length < 1) return reply('yang di cari ??')
					tels = body.slice(8)		
			                if (!isUser) return reply(mess.only.daftarB)
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('linknya mna kak?')
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('di suruh nulis apa kak? kuntull ?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
	...
	case 'kbbi':
            client.updatePresence(from, Presence.composing) 
                if (args.length < 1) return reply(`di suruh nyari apa kak?`)
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'info':
					...
					teks = `*Nama bot* : ${me.name}\n*Nomor bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*block kontak total* : ${blocked.length}\n*bot aktif* : ${kyun(uptime)}\n*chattlist Total* : ${totalchat.length}`
					buffer = await getBuffer(me.imgUrl)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist':
					teks = 'kontak yang kena blocked :\n'
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'dibutuhkan kode bahasa?', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'di butuhkan kode bahasa', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Total list admin di group*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'block':
					...
					client.sendMessage(from, `Suksess.. memblokir kontak ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 TAG SEMUA MEMBER 〙✪══\n╠➥'+teks+'╚═〘 APBOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				if (!isQuotedImage) return reply(`Envie fotos com legendas ${prefix}setbotpp ou tags de imagem que já foram enviadas`)
					...
					reply('makasih foto profil nya omm😗')
					break
				case 'bc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'chord':
					if (args.length < 1) return reply('lagu apa omm?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('lagunya apa omm?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `Hae owner group : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('nomornya mana kak')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('[gagal karena nomornya di private')
					}
					break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`asek... dapet jatah kik 🥳🥳otw Kik : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'asek dapet jatah Kik 😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Textnya kebanyakan omm, máximal 300 huruf!!', msgType.text, {quoted: mek})
                       ...
                       teks1 = `*[RELATED]*\nNomer : @${nomor.split("6282132486898@s.whatsapp.net")[0]}\npesan : ${pesan}`
                      var options = {
                         text: teks1,
			...
			case 'grouptutup':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'DI SURUH² MAGER AJG'})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
            ...
            break
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`texnya mana kak`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`textnya mana omm??`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('[SUKSES] mengaktifkan fitur simi di grup ini✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('[SUKSES] menonaktifkan fitur simi di grup ini ✔️')
					} else {
				...
				case 'clone':
					...
					break
                                /*case 'magernulis':
                                        ...
                                        if (args.length < 1) return reply('teks, nama,mana kak?')
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`Textnya mana umm`)
                                        if (!isUser) return reply(mess.only.daftarB)
